...
* TODO GuixでRustのパッケージを作成する
  :PROPERTIES:
  :EXPORT_FILE_NAME: 2025-560468ab-3c34-2924-400b-71ee3aa40bf4
  :EXPORT_HUGO_TAGS: GNU-Guix Scheme
  :END:
いままでRustのパッケージはすべてGuixのパッケージと一対一対応していた。
そのため、あるRustパッケージをビルドするためには、 =#:cargo-inputs= として引数に渡されている依存パッケージを全てビルドする必要があった(たしかビルドしないというオプションはあったはずだが)。
一方Rustのビルドプロセス上、全ての依存はソースコードを利用してコンパイルされるため、依存パッケージがバイナリへビルドされている必要は全くなかった。
また、Rustパッケージに対応するGuixパッケージの数も依存の数に応じてどんどん増えていってしまうなど、管理上の問題も多数あった。

しかし[[https://guix.gnu.org/blog/2025/a-new-rust-packaging-model/][2025年にその方針が改められ、バイナリが真に必要なRustパッケージ以外は全てソースコードのみの依存とすることとなった]]。
...
** 前提条件
チャンネルは既に保持しているものとする。
もし持っていない場合は、[[https://blog.rocktakey.com/information-science/2022-770eb4b6-5c63-62d4-bbcb-475d92404603/][Guixのパッケージをつくってみよう-その1: 簡単なフォントのパッケージを作る]]などを参考に作成して欲しい。
...
- ~guix import --insert=/path/to/mychannel/packages/rust-apps.scm crates <PACKAGE名>~ を実行する。
- ~(cargo-inputs <PACKAGE名>)~ を ~(cargo-inputs <PACKAGE名> #:module '(mychannel packages rust-crates))~ に置き換える。
...
このファイルは初めてRustパッケージを作成するときに一度だけ用意すればよく、以後は使いまわしてよい。
...
=/path/to/mychannel/packages/rust-crates.scm= は先程[[#create-rust-crates-file]]で用意したファイルのパスで、
=/path/to/Cargo.lock= は今取得した =Cargo.lock= へのパスである。
#+begin_src shell
...
一応説明しておくと、現時点でTODOコメントは二種類ある。
- Define standalone package if this is a workspace. ::  crate.ioからではなくgitを取得している場合に出る。
  GitレポジトリがRustのworkspaceを用いている場合、欲しいクレートを明示的に指定しないとクレートとして利用できないため、
  独立したパッケージとして記述しなければならないということのようだが、私自身そうしなければならない状況になったことがなく、
  最終的にどうするのかはあまりよくわからない(そのパッケージは =rust-crates.scm= に置いてよいのか、 =define-cargo-inputs= に指定してよいのか、それとも別のファイルに置くのか？)。
...
違いは =--lock-file= の指定がない点と、 =--insert= する先が違う点のみだ。
#+begin_src shell
...
すると、次のようなパッケージ定義がファイルの末尾に挿入される。以下は[[https://github.com/ROCKTAKEY/roquix/blob/22342c12404447746d21996d042e730747ad0a85/roquix/packages/texlab.scm][私のチャンネルにある]] =texlab= というパッケージの例だ。
#+begin_src scheme
...
そのために以下をファイルの先頭に記述する。 =(mychannel packages filename)= の部分はあなたのチャンネルのルートからの相対パスに合わせて欲しい。
#+begin_src scheme
...
先程 =lookup-cargo-inputs= を定義したが、それをどのモジュールにあるか教えなければ当然依存関係を辞書から引くことすらできない。
...
=#:install-sources #f= はここでは関係ないので無視してよい。
=modify-phases= の文法について詳細に説明するのがこの記事の趣旨ではないので、[[https://blog.rocktakey.com/information-science/2022-c5b823db-a6b6-23a4-5783-c7741fd0f420/#%e4%be%9d%e5%ad%98%e3%81%ae%e8%a7%a3%e6%b1%ba][私が前に書いたGuixパッケージ定義を作成する記事]]に委ねるとして、 =substitute*= について軽く説明する。
...
=main.rs= を含むディレクトリはバイナリを生成するので、そこからあたりを付けるのがいいだろう。
...
*** テストが失敗する
テストの失敗理由はいろいろあるが、ネットワークが必要な場合など、Guixでは不可避なものもある。
