...
後者の場合、異なるコミットやタグにチェックアウトしている場合は異なるハッシュが返るので注意してください。
また、裏技として、適当なハッシュを入れてパッケージをビルドすると「ハッシュが間違ってるよ」と言いながら実際のハッシュを教えてくれます。
...
* TODO Guixのパッケージをつくってみよう-その2: ビルドが必要なパッケージを作る
  :PROPERTIES:
