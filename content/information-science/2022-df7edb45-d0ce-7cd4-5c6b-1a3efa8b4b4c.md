...
lastmod = 2022-12-22T17:56:19+09:00
...
-   [keg.el](https://github.com/conao3/keg.el)
    ...
    -   類似ツールとして[Cask](https://github.com/cask/cask)、[eldev](https://github.com/doublep/eldev)、[eask](https://github.com/emacs-eask/cli)などがある
...
ドキュメントは[ここ](https://www.gnu.org/software/emacs/manual/html_node/ert/)にあります。
...
その他のオプションの詳細は[undercover.elのREADME](https://github.com/undercover-el/undercover.el)をご覧ください。
...
このファイルでは[keg.el](https://github.com/conao3/keg.el)というツールのプロジェクト毎の設定が書かれています。
この節では `keg.el` に限らず、Emacsプロジェクトにおいて導入しておくと便利なCLIツールについて書いています。

ここでは一旦例として `keg.el` を用いますが、他も大抵の仕組みは同じだと思います。
他には[cask](https://github.com/cask/cask)が有名です。
...
それぞれ[GNU ELPA](https://elpa.gnu.org/)、[NonGNU ELPA](https://elpa.nongnu.org/)、[MELPA](https://melpa.org/#/)から落とすことを指定しています。
...
このあたりは若干方言がありますが、基本文法は[MELPAの説明](https://github.com/melpa/melpa#recipe-format)がわかりやすく、よく用いられています。
`plist` のうち、代表的なものを以下に示します。

`:fetcher`
: ソースコードをどこから取ってくるかです。今回はGitHubでホストしているので `github` としています。
...
`test-all`
: こちらでは、 `keg-shell` という1つの関数のみを呼び出しています。
...
: パッケージとしての要件を満たしているかを検証してくれます。Emacsの最低バージョンで対応していない関数を利用してしまっていないか、
    ...
    詳しくは[package-lintのGitHub](https://github.com/purcell/package-lint)及び[Emacs Lispのコーディング規約](https://www.gnu.org/software/emacs/manual/html_node/elisp/Coding-Conventions.html)をご覧ください。
...
`checkdoc`
: ドキュメント周りの問題を指摘してくれます。引数がすべて入っているか、ピリオドの後にスペースが2つ入っているか、
    ドキュメントの各行が80桁以下になっているか、などを確認します。
    詳細は [checkdoc.elのソースコード](https://github.com/emacs-mirror/emacs/blob/master/lisp/emacs-lisp/checkdoc.el)の説明文(Commentary)をご覧ください。
    なお、これは標準搭載のパッケージなので、インストールは必要ありません。
    もし今利用しているEmacsで利用したい場合は、該当バッファで `checkdoc` コマンドを実行してください。

`byte-compile`
: コンパイルしたときにエラーや警告が出ないかどうかを確認します。
...
コメントやメール、[Emacs JP](https://emacs-jp.github.io/)のSlackなどでなんなりとご連絡ください。
