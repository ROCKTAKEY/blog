...
lastmod = 2020-09-06T14:52:19+09:00
...
`tab-line-mode` では、[Figure 1](#org79185bf)にあるように、
ウィンドウ[^fn:1]
...
を返す `tab-bar-tab-name-current` ([Figure 3](#orgefdf57a))となっているが、
任意の関数に変更可能。
windowの数を数えて付け加えてくれる `tab-bar-tab-name-current-with-count`
([Figure 4](#org9927b36))、
長さが `tab-bar-tab-name-truncated-max` を超えると省略してくれる
`tab-bar-tab-name-truncated` ([Figure 5](#org72212e2))、
表示されている全バッファの名前をカンマで繋げてくれる
`tab-bar-tab-name-all` ([Figure 6](#orgb445b67))が用意されている。
自身で作った関数でももちろんOK。
