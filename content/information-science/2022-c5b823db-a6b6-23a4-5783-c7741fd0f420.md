...
lastmod = 2022-12-07T12:37:28+09:00
...
また、ブラウザでは[こちら](https://packages.guix.gnu.org/)で検索することも可能です。
...
すると以下のような表示が得られます。
ブラウザの検索の場合は[こちら](https://packages.guix.gnu.org/packages/pkg-config/0.29.2/)をご覧ください。
...
公式レポジトリでいうと[こちら](https://git.savannah.gnu.org/cgit/guix.git/tree/gnu/packages/pkg-config.scm#n37)になります。そこには以下のような定義が書いてあります。
...
また、もう一度基本に戻って[公式のビルド手順](https://github.com/ggreer/the_silver_searcher#building-master)を見てみると、Fedoraにおける依存解決のしかたが書いてあります。
...
なくても動きますが、[公式のチャンネル](https://git.savannah.gnu.org/cgit/guix.git/tree/gnu/packages/code.scm#n338)の定義では入れています。
...
インポーターの一覧など、詳しくは[公式マニュアル(英語)](https://guix.gnu.org/en/manual/devel/en/guix.html#Invoking-guix-import)にありますが、例えば以下のようにしてパッケージを生成できます。
...
また、Guixの日本語コミュニティである[Guix-jp](https://guix-jp.gitlab.io/)もあります。
